...
function handlerDblClick(e) {
    ...
    if (!targetCell) // eslint-disable-next-line
    return;
...
function cellValid(input) {
    ...
    if (currentIndex === 0 && value.length < 4) return {
        type: "warning",
        text: "Name can not be less then 4 letters"
    };
    else if (currentIndex === 3 && +value < 18) return {
        type: "warning",
        text: "Age can not be less then 18"
    };
    else if (currentIndex === 3 && +value > 90) return {
        type: "warning",
        text: "Age can not be bigger then 90"
    };
    return {
        type: "success",
...
function sorting(e) {
    const arrOfTitleCells = [
        ...tHeadRow.querySelectorAll("th")
    ];
    const sortIndex = arrOfTitleCells.indexOf(e.target);
    if (sortIndex === -1) // eslint-disable-next-line
    return;
    ...
    arrOfRows.sort((row1, row2)=>{
        ...
        switch(typeof cellValue1){
            case "string":
                if (isDesc) return cellValue2.localeCompare(cellValue1);
                return cellValue1.localeCompare(cellValue2);
            default:
                if (isDesc) return cellValue2 - cellValue1;
